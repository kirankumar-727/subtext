...
begin
  ...
  insert into public.article_media(revision_id,media_asset_id,role,position,alt_text)
  select new_revision_id,selected.media_asset_id,'inline',
         row_number() over(order by selected.ord)::integer-1,asset.default_alt_text
  from (
    select media_id as media_asset_id,ord
    from unnest(coalesce(p_media_asset_ids,'{}'::uuid[])) with ordinality as media_id(media_id,ord)
    where media_id<>p_cover_media_asset_id
  ) selected
