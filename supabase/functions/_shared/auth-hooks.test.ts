...
Deno.test("signed Auth hook payload verifies before policy evaluation", async () => {
  ...
  Deno.env.set("TEST_AUTH_HOOK_SECRET", `v1,whsec_${bytesToBase64(secret)}`);
  const payload = JSON.stringify({ user: { email: "founder@subtext.media" } });
...
Deno.test("founder policy admits Google founder and grants admin claim", () => {
  const founderEmail = "founder@subtext.media";
  const app_metadata = { provider: "google", providers: ["google"] };
  assertEquals(
    decideBeforeUserCreated({ user: { email: founderEmail, app_metadata } }, founderEmail),
    {},
  );
  assertEquals(
    decideCustomAccessToken(
      { claims: { email: founderEmail, app_metadata, role: "authenticated" } },
      founderEmail,
    ).claims.user_role,
    "admin",
  );
