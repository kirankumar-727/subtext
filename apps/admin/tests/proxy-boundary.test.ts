...
import {
  createAdminProxyHandler,
  type SessionRefresher,
} from "@/lib/auth/proxy-handler";
...
function refresher(claims: AuthClaims | null): SessionRefresher {
  return (async (incomingRequest: NextRequest) => {
    ...
    response.cookies.set("session-refresh-test", "preserved", {
      httpOnly: true,
    });
...
describe("admin proxy security boundary", () => {
  ...
  it("redirects an unauthenticated direct /admin request to login", async () => {
    const response = await createAdminProxyHandler(refresher(null))(
      request("/admin"),
    );
    ...
    expect(response.headers.get("location")).toBe(
      "https://admin.subtext.media/login",
    );

    expect(response.cookies.get("session-refresh-test")?.value).toBe(
      "preserved",
    );
  });
  ...
  it("redirects an authenticated unauthorized account before rendering", async () => {
    ...
    expect(response.headers.get("location")).toBe(
      "https://admin.subtext.media/access-denied",
    );
  });
  ...
  it("redirects an already-authorized founder away from login", async () => {
    ...
    expect(response.headers.get("location")).toBe(
      "https://admin.subtext.media/admin",
    );
  });
});